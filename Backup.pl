...
sub do_snapshots
{
        ...
        if (scalar(keys(%snaps_ts_key)) < $_->{'quantity'} && (($now - $last_snap) >= $_->{'freq'}))
	{
	    ...
	    $snapshot = $$ec2->create_snapshot(-volume_id=>$_->{'volumeId'},-description=>"Created automatically by instance $ref_cfg_info->{'me'} from volume $_->{'volumeId'}");
	...
	if ( (scalar(keys(%snaps_ts_key)) == $_->{'quantity'}) && (scalar(keys(%snaps_ts_key)) >= 1))
	{
		...
		## Create
    	        my $snapshot = $$ec2->create_snapshot(-volume_id=>$_->{'volumeId'},-description=>"Created automatically by instance $ref_cfg_info->{'me'}  from volume $_->{'volumeId'}");
                (_log($ref_cfg_info->{'logfile'}, 'warn', "Error creating snapshot API: $$ec2->{'aws'}->{'error'}\n"), next) if (defined($$ec2->{'aws'}->{'error'}));
